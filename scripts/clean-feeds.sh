...
#########################################
echo "=== 复制 luci-base ==="
cp -r "$SRCLUCI/libs/luci-base" "$DESTFEEDS/"
...
cp -r "$SRCLUCI/modules/"* "$DESTFEEDS/modules/"
...
cp -r "$SRCLUCI/collections/"* "$DEST_FEEDS/collections/"
...
cp -r "$SRCLUCI/i18n/zhHans" "$DEST_FEEDS/i18n/"
...
CONFIGFILE="$GITHUBWORKSPACE/repo/sl3000/config/sl3000.config"
...
for key in DEVICEVENDOR DEVICEMODEL DEVICEVARIANT DEVICEDTS DEVICE_PACKAGES IMAGES; do
...
for cfg in CONFIGTARGETmediatek=y CONFIGTARGETmediatek_filogic=y; do
...
for f in "$DTSFILE" "$MKFILE" "$CONFIG_FILE"; do
