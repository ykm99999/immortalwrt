...
# 1. 🔥 [物理修复] 恢复默认源索引，确保 base 等核心组件不丢失
# 解决 "find: feeds/base: No such file or directory" 报错
git checkout feeds.conf.default || true

# 2. 执行物理更新，拉取原始索引
./scripts/feeds update -a

# 3. 🔥 [旗舰级专属致盲] 在安装前物理删除所有干扰包
# 这才是彻底解决“全量安装”的终极手段：删掉文件夹，install -a 就变成了专属安装
echo "Executing physical blinding of redundant packages..."
